...
```
AutoPilot (Project 1)          TestPilot (Project 2)
──────────────────────         ──────────────────────
Java                           Python
Selenium                       Playwright
...
## How The Three Layers Work Together — Critical Concept

**Every Claude coaching TestPilot must understand this before explaining anything.**

There are three distinct layers. They are completely different things. Confusing them
leads to wrong explanations. Understand each one separately.

```
LAYER 1 — THE HUMAN (You)
Writes test cases in Excel. Plain English.
"Step 3: click the login button"
Decides WHAT needs to happen and WHY.
Never writes Python code for test cases.
Never touches the browser directly.

        ↓ Excel file passed to agent

LAYER 2 — THE AGENT BRAIN (Groq / LLM)
Reads the Excel step.
Observes the current live page via perception.py.
DECIDES which Playwright action to call.
Returns structured JSON: "call click_element with selector=#login-button"
Does NOT call Playwright directly.
Does NOT touch the browser.
Only returns a decision as text.

        ↓ Decision (JSON) passed to Python code

LAYER 3 — PLAYWRIGHT (The Hands)
Receives the agent's decision via Python code.
Actually opens the browser.
Actually finds the element.
Actually clicks, types, navigates.
Has zero intelligence — only executes what it is told.
```

**The exact mechanism — what happens in code:**

Step 1: Python sends this to Groq:
```
"Step: click the login button.
 Page has: [button] id=login-button text='Login'
 Available tools: click_element, type_text, navigate, assert_visible..."
```

Step 2: Groq returns this (just JSON text — no browser action):
```json
{"tool_call": {"name": "click_element", "arguments": {"selector": "#login-button"}}}
```

Step 3: Your Python code reads that JSON and calls Playwright:
```python
if tool_name == "click_element":
    page.locator(selector).click()   # THIS is what actually clicks the browser
```

**The AI never touches Playwright. The AI only returns a decision.**
**Python is the bridge that reads the decision and calls Playwright.**
**This is what "tool use" or "function calling" means.**

**The surgeon analogy (use this when explaining):**
The surgeon (Groq) decides what to cut and where. The surgeon does not cut with bare hands.
The nurse (your Python code) reads the surgeon's decision and hands over the right scalpel.
The scalpel (Playwright) does the actual cutting. Remove any one of the three and the
operation fails.

**Why this architecture is correct:**
The AI is good at reasoning over ambiguous text and deciding intent.
Playwright is good at reliably controlling a browser with precision.
Neither can do the other's job. Combining them gives you both.
...
present. Before every interaction Playwright checks: is the element visible, stable,
enabled, in viewport, and receiving pointer events? All five must pass before it acts.
This eliminates entire categories of timing flakiness. Sandeep already understands
Selenium's concepts. Playwright is the same mental model, cleaner API. Career value:
Playwright appears on every modern SDET job description.
...
## Agent Reliability — What Every Claude Must Know

**This section must be understood before coaching any phase from Phase 3 onwards.**

### The agent does not learn during runs

The Groq model is frozen. Its weights were set during training and do not change when
you call it. It has no memory of yesterday's run. It does not improve automatically.

This surprises people. The correct mental model: a calculator does not learn that you
keep making arithmetic mistakes. It just calculates. The model just reasons over whatever
you put in front of it right now.

**How TestPilot gets smarter without the model learning:**

Three mechanisms — all in the architecture:

1. **Better prompts**: if the agent makes a recurring mistake, improve the system prompt.
   One prompt change improves every future run. The model did not change. Input quality did.

2. **Better perception**: if the agent makes wrong decisions, it usually saw a bad page
   description. Improve perception.py. Better input = better decisions.

3. **Run history as context**: inject relevant past failures into the prompt before each
   step. "This exact step failed 3 times last week with this error." The model reasons
   differently because you gave it memory. Not learning — contextual awareness.

**Fine-tuning (the real "learning") — what it is and when it applies:**

Fine-tuning means retraining a model on your specific data so its weights actually change.
It makes the model genuinely better at your specific domain.

The honest verdict for TestPilot right now:
- Fine-tuning requires thousands of real input-output examples
- TestPilot has not run a single test case yet — zero training data exists
- The correct sequence: build → run → collect data → improve prompts → fine-tune if needed
- Skipping to fine-tuning now is building a race car before knowing which road you drive on

Fine-tuning is a real future upgrade path. It is not in scope for v1. Build first.
Measure. Then improve with evidence.

### Realistic reliability target

A well-tuned TestPilot on SauceDemo:
```
~80-90% on first build
~90-95% after improving perception and prompts
Never 100% — the model is probabilistic by nature
```

This is not a failure. It is the honest nature of non-deterministic systems.
The eval harness measures this number. Showing the number and the trend is the
interview story. "It works because I measured it" beats "it works, trust me."

### The four break points and how each is handled

**Break Point 1 — Groq returns the wrong tool call:**
Groq is probabilistic. Same input does not always produce same output. Occasionally
it picks the wrong element or wrong action. Self-healing catches this — re-observes
the page and asks Groq to try again with fresh context. Better prompts reduce frequency.

**Break Point 2 — Groq returns a valid tool but selector does not exist:**
self_healer.py catches this. Re-observes the page, asks Groq for alternative selector,
tries again. Two attempts before escalating to human in the loop.

**Break Point 3 — Playwright finds element but page is not ready:**
Playwright's five-condition actionability checking handles this automatically. It waits
for elements to be stable before acting. STEP_TIMEOUT=15 in .env gives 15 seconds for
elements to become actionable. For edge cases that slip through, healing catches them.

**Break Point 4 — Expected result check is wrong (bad test step):**
The agent cannot fix this. If the expected result is vague, the agent cannot know intent.
Prevention: TEST_CASE_AUTHORING_GUIDE.md teaches precise expected results.
Detection: triage classifier flags BAD_STEP classification.
This is a human problem, not an agent problem.

---

## Perception Layer — Architecture and Known Limitations

**Read this before building perception.py on Day 15.**

### How perception works

The agent cannot receive raw HTML — a typical page is 50,000+ characters.
perception.py extracts a compact, structured summary the agent actually reasons over.

**Three-layer perception strategy (build in this order):**

**Layer 1 — DOM extraction (primary, handles ~95% of real apps):**
```python
page.query_selector_all("input, button, a, select, textarea")
# Extract: tag, id, name, placeholder, aria-label, visible text, type
```

**Layer 2 — Accessibility tree (secondary, handles custom components):**
```python
page.accessibility.snapshot()
# Returns role, name, value, description for every accessible element
# Works on some custom web components invisible to DOM queries
# Add this as a secondary read alongside DOM extraction
```

**Layer 3 — Vision fallback (tertiary, for sparse DOM situations):**
```python
# When DOM + accessibility tree return fewer than 3 elements:
screenshot = page.screenshot()
# Pass to vision model to describe what is visible
# Less precise but handles edge cases DOM cannot see
# Build this in Phase 7 if time permits — not Phase 2
```

**The default perception.py must implement Layer 1 + Layer 2.**
Layer 3 is a Phase 7 stretch goal.
...
### Known limitation — canvas and WebGL apps

Some applications render their entire UI inside a `<canvas>` element using JavaScript.
The canvas has no DOM structure — it is painted pixels. Playwright sees one element.

```html
<!-- What Playwright sees on a canvas-heavy app -->
<canvas id="app-canvas"></canvas>
<!-- The login button is painted pixels inside this — invisible to DOM queries -->
```

**How common is this:**
```
Standard web apps (React, Vue, Angular, plain HTML): No limitation. Works perfectly.
This covers LinkedIn, Gmail, Amazon, banking portals, HR systems, e-commerce.
Most SDET jobs never encounter canvas-heavy apps.

Partially canvas apps (charts, maps inside normal DOM): Only the canvas component
is limited. The rest of the page works fine.

Fully canvas apps (Figma, WebGL tools, trading terminals): Full limitation.
These are a small minority — maybe 5% of web apps in typical SDET work.
```

**SauceDemo does not have this problem. This limitation does not affect the project.**

**The interview answer for this limitation:**
*"TestPilot's perception layer reads the DOM and accessibility tree, which covers the
vast majority of standard web applications. For canvas-heavy applications like Figma
or WebGL-based tools, DOM perception has limits — the UI is painted pixels, not
structured elements. I know this boundary exists. The architecture supports adding
a vision-based fallback for those cases, but I scoped v1 to DOM-accessible apps
because that covers most real SDET work and keeps the reliability story clean.
Knowing where your tool works and where it does not is part of being a trustworthy engineer."*
...
│   └── perception.py        ← How the agent "sees" the page (DOM + accessibility tree)
│
├── data/
│   ├── __init__.py
...
2. Observe the current page state (via perception.py — DOM + accessibility tree)
3. Build a prompt: "Here is the step. Here is what the page looks like. What tool should I call?"
4. Send to Groq — get back a tool call decision (JSON, not an action)
5. Python code reads the JSON and executes the tool call via tools.py
6. Playwright physically performs the browser action
7. Check if the step's expected result is satisfied
8. If yes — move to next step
9. If no — trigger self_healer.py
10. If healing fails twice — trigger human_in_the_loop pause
11. Record everything for the report
...
The AI returns a decision. Python executes the decision. Playwright performs the action.
Three layers. Never collapse them into one in explanations.
...
**Ollama note:** Ollama is not a model — it is a tool that runs open-source models
locally. Llama 3.1 8B through Ollama is the same model quality as Llama 3.1 8B through
Groq. The difference is that Groq is a hosted API (real API key, real rate limits,
real production patterns) while Ollama runs on localhost with no API key needed.
Sandeep's 16GB RAM machine runs Ollama comfortably.
...
- The current page's simplified DOM + accessibility snapshot (from perception.py)
- The current URL
- Previous step results for context
- Relevant run history if available (for recurring failure patterns)
...
- Browser context isolation — each test case starts with a clean session

### `browser/perception.py`
**How the agent "sees" a page — critical component.**

**Must implement two layers:**

Layer 1 — DOM extraction:
```python
elements = page.query_selector_all("input, button, a, select, textarea")
# Extract per element: tag, id, name, placeholder, aria-label, visible text, type
...
# Adds role, name, value for elements custom components might hide from DOM
# Run alongside DOM extraction, merge results, deduplicate
```
...
TITLE: Swag Labs
INTERACTIVE ELEMENTS:
  [input] id=user-name placeholder="Username" role=textbox
  [input] id=password placeholder="Password" type=password role=textbox
  [button] id=login-button text="Login" role=button
  [a] href="/inventory.html" text="Sauce Labs Backpack" role=link
```

This is what the agent actually reasons over. Clean, compact, enough.

**Why both layers matter:**
DOM alone misses some custom web components. Accessibility tree alone may miss
some standard elements. Together they cover the full picture for standard web apps.

**Canvas limitation:** If DOM + accessibility tree return fewer than 3 elements,
the page may be canvas-heavy. Log a warning. Vision fallback is a Phase 7 stretch goal.
...
1. Re-observe the page fresh — DOM + accessibility tree again
2. Ask Groq: "This selector failed. Here is the current page. Suggest an alternative."
3. Try the alternative
4. If that fails — ask Groq with more context about the original intent, try again
5. If both fail — trigger human in the loop
...
- Failure breakdown by type (HTML/CSS bar chart — no library needed)
...
- prompts.py (injecting history context to reduce recurring failures)
...
| DOM + accessibility tree in perception.py | DOM alone misses custom components; both layers together cover standard web apps completely |
| Compact perception output | Raw HTML is too large for context window; structured extract is enough |
...
| Eval harness built deliberately | The reliability story is the interview centrepiece; measurement is the credibility layer |
...
| No fine-tuning in v1 | Zero training data exists yet; build first, collect data, improve with evidence |
...
- Standard DOM-based React app — no canvas limitations, perception layer works perfectly
...
- Concept: the AI returns a decision as JSON — your code executes it. Not the AI.
...
- Verify the response is JSON containing the tool name and arguments — not a browser action
...
- The five actionability conditions Playwright checks before every action:
  visibility, stability, enabled state, viewport presence, pointer event reception
...
call back, Python code executes it on SauceDemo. One step. End to end. Real.
...
- What is function calling? The model returns structured JSON saying "call this function
  with these arguments" — your Python code executes it. Not the model.
- The three layers again: Human (Excel) → Agent Brain (Groq decision) → Playwright (action)
- Analogy: surgeon decides, nurse hands the scalpel, scalpel cuts. Three separate roles.
- Exercise: read Groq's function calling documentation. Identify: where do you define tools,
  where do you send them, what does the response look like when the model decides to call one.
...
- Verify: Groq returned JSON. It did not click anything. Your code will do that.
...
- Observe: get current page state (perception.py output — DOM + accessibility tree)
- Decide: send state + instruction to Groq, get tool call JSON back
- Act: Python reads the JSON, calls Playwright, browser performs the action
...
- Exercise: draw the loop on paper with boxes and arrows. Label every input and output.
  Label which layer each box belongs to: Human / Agent Brain / Playwright.
...
- For each step: calls perception.py to get page state, calls Groq to decide the tool,
  Python code executes the Playwright action
...
- Why two layers: DOM extraction + accessibility tree snapshot
...
Part 2 — Build perception.py (DOM + accessibility tree — both layers required)
- Layer 1: use `page.query_selector_all()` to get all interactive elements
  Extract: tag name, id, name, placeholder, aria-label, visible text, type
  Filter out invisible elements
- Layer 2: use `page.accessibility.snapshot()` to get accessibility tree
  Extract: role, name, value — merge with DOM results, deduplicate
- Format as a compact string the prompt can include
- Add a guard: if fewer than 3 elements found, log a warning (possible canvas app)
...
- Exercise: trace a single test step through every module on paper. Input and output at
  each boundary. Label which layer (Human/Brain/Playwright) each module belongs to.

Part 2 — Build agent/core.py
- The run_test_case(test_case: TestCase) method
- For each step: observe (perception), build prompt, call Groq, parse tool call JSON,
  Python executes Playwright action, check expected result
...
- Attempt 1: re-observe page (DOM + accessibility tree), ask Groq for alternative selector
...
- How run history feeds the prompts: recurring failures get injected as context
...
- Read history in prompts.py to inject context for recurring failures
...
**Day 25 — Generalisability Test**

Part 1 — Testing on an unknown site (the most compelling demo moment)
- Why this matters: proves the agent does not have SauceDemo hardcoded anywhere
- The architecture is general: Excel test cases → agent perceives live page → executes
- Choose the second site (The Internet on Herokuapp or DemoQA)
- Write 5 test cases for it in Excel — no code changes anywhere in TestPilot
- Run TestPilot against it
- Document honestly: what worked, what did not, what the limitations revealed
- Honest documentation is itself impressive — it shows engineering thinking, not salesmanship
- Note: confirm the second site is standard DOM-based before running. Check for canvas
  elements in browser DevTools first.

Part 2 — Vision fallback stretch goal (if time permits)
- If the second site has sparse DOM on any page, implement the vision fallback:
  screenshot → pass to Groq vision → describe elements → use as perception output
- This is not required. It is the natural next evolution of perception.py.

**Day 26 — CI/CD, Authoring Guide, README**

Part 1 — CI/CD concepts
- Same GitHub Actions concepts as AutoPulse
- Headless mode for CI runs (HEADLESS=true in CI environment)
- Groq API key as a GitHub secret
- Run eval harness on every push — catch regressions automatically

Part 2 — Documentation
- Add GitHub Actions workflow
- Write TEST_CASE_AUTHORING_GUIDE.md:
  what actions are supported, how to write targets the agent understands,
  how to write precise expected results (not vague ones), examples of good and bad steps
- Polish README: what TestPilot is, how to run it, what the eval results show
- Update README badge with CI status

**Day 27 — Final Polish and Interview Prep**

Part 1 — Clean the codebase
- Remove debug print statements
- Ensure all error paths are handled
- Run the full test suite — everything green
- Run the eval harness — record the final pass rate number

Part 2 — Interview preparation
- Practice the elevator pitch out loud (not just read it)
- Practice the eval results story — Option C — with the real numbers
- Prepare answers for every talking point in this document
...
*"Playwright has superior auto-waiting. Before interacting with any element, it checks
five actionability conditions — visibility, stability, enabled state, viewport presence,
and pointer event reception. All five must pass before Playwright acts. Selenium requires
you to write explicit waits for most of these. For an agent executing steps autonomously,
Playwright's auto-waiting removes an entire category of flakiness that would otherwise
require the agent to reason about timing. I also wanted Playwright experience specifically
because it appears on every modern SDET job description."*
...
**"Does your agent learn from its mistakes?"**:
*"The model itself does not learn during runs — it is a frozen model, same weights every
call. But the system gets smarter in three ways: better prompts based on observed failure
patterns, richer perception that gives the model better page context, and run history
injected into prompts so the agent has contextual awareness of recurring failures. The
next evolution would be fine-tuning — retraining on real run data collected from production
use. But fine-tuning requires thousands of quality examples, which only exist after the
system has been running for a while. Build first, collect data, improve with evidence."*
...
**"How does the AI actually control the browser?"**:
*"The AI does not control the browser directly. There are three distinct layers. The human
writes test cases in Excel — that is the intent layer. Groq reasons over the current page
state and the test step, and returns a structured JSON decision: 'call click_element with
this selector.' That is the reasoning layer. My Python code reads that JSON and calls
Playwright, which physically clicks the element in the browser. That is the execution layer.
The AI never touches Playwright. It only returns a decision. Python is the bridge."*
...
*"TestPilot's perception layer reads the DOM and accessibility tree, which covers the
vast majority of standard web applications. For canvas-heavy applications like Figma
or WebGL-based tools, DOM perception has limits — the UI is painted pixels, not structured
elements. I know this boundary exists. The architecture supports adding a vision-based
fallback for those cases, but I scoped v1 to DOM-accessible apps because that covers most
real SDET work and keeps the reliability story clean. Knowing where your tool works and
where it does not is part of being a trustworthy engineer."*
...
- **He asks deep questions** — when he asks about fine-tuning, RAG, canvas limitations, or agent architecture, answer completely. Do not deflect. He is building genuine understanding.
...
14. **The AI returns a decision. Python executes it. Playwright performs it.** — three layers, never collapse them
15. **Build first, measure, then improve with evidence** — do not fine-tune before you have data
